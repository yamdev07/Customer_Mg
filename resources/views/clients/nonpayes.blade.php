...
<div class="container-fluid px-3 px-md-4 py-4">
    ...
    <div class="page-hero mb-4 ax-fade-up">
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 position-relative" style="z-index:1">
            <div>
                <h1 class="h3 fw-bold mb-1"><i class="fas fa-times-circle me-2"></i>Clients non payés</h1>
                <p class="mb-0 opacity-75">Abonnements en attente de paiement</p>
            </div>
            <span class="badge bg-white text-anyxtech fs-6 px-3 py-2">
                <i class="fas fa-exclamation-triangle me-2"></i>{{ $nonPayes }} non payés
            </span>
        </div>
    </div>
    ...
    <div class="card mb-4">
        <div class="card-body p-3 p-md-4">
            <div class="row align-items-center g-3">
                <div class="col-lg-7">
                    <form method="GET" action="{{ route('clients.nonpayes') }}" class="position-relative">
    ...
    @if ($clients->isEmpty())
        <div class="card">
            <div class="card-body text-center py-5 text-muted">
                ...
                <h5 class="fw-semibold">Aucun client non payé trouvé</h5>
                <p class="mb-0">Tous les clients ont régularisé leur situation.</p>
            </div>
        </div>
    @else
        <div class="card">
            <div class="card-header bg-white border-0 p-3 p-md-4 d-flex align-items-center">
                <h5 class="mb-0 fw-semibold"><i class="fas fa-times-circle me-2 text-danger-600"></i>Liste des clients non payés</h5>
                                    ...
                                    <td>
                                        @php
                                            ...
                                            $badgeClass = match($statut) {
                                                'actif' => 'badge bg-success-light text-success-600',
                                                'suspendu' => 'badge bg-warning-light text-warning-600',
                                                default => 'badge bg-anyxtech-light text-anyxtech',
                                            };
                                        @endphp
                                    ...
                                    <td class="pe-4">
                                        <div class="d-flex gap-2 justify-content-end">
                                            <form action="{{ route('clients.reconnecter', $client->id) }}" method="POST" onsubmit="return confirm('Confirmer la reconnexion du client ?')">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="btn btn-success btn-sm" title="Reconnecter">
                                                    <i class="fas fa-plug me-1"></i>Reconnecter
                                                </button>
                                            </form>
                                        </div>
                                    </td>
